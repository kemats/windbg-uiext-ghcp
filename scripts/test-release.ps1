...
foreach ($invalid in @('../escape', '01.2.3', '1.2', '1.2.3-01')) {
    $rejected = $false
    try { & $releaseScript -Version $invalid -Architecture $Architecture } catch { $rejected = $true }
    if (!$rejected) { throw "Invalid version accepted: $invalid" }
}
$mismatch = $false
try { & $releaseScript -Version '65534.65534.65534' -Architecture $Architecture } catch {
    if ($_.Exception.Message -notlike 'Version mismatch*') { throw }
    $mismatch = $true
}
if (!$mismatch) { throw 'Mismatched assembly version was accepted.' }
$archive = Join-Path $root "artifacts/release/WinDbgCopilotChat-$Version-$rid.zip"
...
try {
    ...
    foreach ($required in @('scripts/install.ps1', 'scripts/uninstall.ps1', 'README.md', 'docs/design.md',
        ...
        "artifacts/package/WinDbgCopilotChat/core/runtimes/$rid/native/copilot.exe",
    ...
    if ($metadata.version -cne $Version -or $metadata.architecture -ne $rid) { throw 'Invalid release metadata.' }
} finally { $zip.Dispose() }
