...
test('native bridge rejects stale snapshots and does not render raw HTML or images', async ({ page }, testInfo) => {
  ...
  const accountLabel = page.getByLabel('GitHub account').locator('span')
  await expect(accountLabel).toHaveText('@a-long-github-account-name-for-layout')
  expect(await accountLabel.evaluate(element => element.scrollWidth)).toBeLessThanOrEqual(await accountLabel.evaluate(element => element.clientWidth))
  await page.getByRole('button', { name: 'Session info', exact: true }).click()
  const info = page.getByRole('region', { name: 'Session info' })
