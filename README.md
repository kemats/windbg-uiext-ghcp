...
User messages are right-aligned and assistant responses are left-aligned without You/Copilot labels. Hover or keyboard-focus a completed response to reveal its turn start time, reported models and credits, alongside copy, open-in-browser and read/stop controls. Touch devices show these controls without hover. Empty tool-only responses are hidden.

**Open response in browser** serves a script-free report from an in-process server bound only to `127.0.0.1` and opens its random 256-bit URL in the default browser. **Open response in Immersive Reader** starts Microsoft Edge with the same kind of report URL prefixed as `read:http://...`, following [Microsoft's documented manual URL form](https://support.microsoft.com/ja-jp/education/learning-accelerators/use-immersive-reader-in-microsoft-edge); it reports an error when Edge is not installed in a supported per-machine or per-user location. Markdown is rendered into semantic headings, paragraphs, lists, tables and code blocks. Mermaid diagrams already rendered by the chat UI are revalidated and embedded as `data:image/svg+xml` images, remaining resolution-independent while preventing reader mode from reconstructing their internal SVG layout. Code and call stacks remain selectable, copyable HTML in both report modes. The Immersive Reader report adds standard `aria-hidden="true"` metadata to code blocks as a best-effort accessibility hint; Edge does not document an element-level Read Aloud exclusion contract, so behavior can vary by Edge version. Raw response HTML is disabled. Reports remain in memory for up to one hour, are capped at 32 recent entries, use `no-store`, and are cleared when WinDbg exits; no report HTML is written to disk. The `article#immersive-reader-content` shape follows the separate Azure Immersive Reader SDK's input guidance. Loopback delivery enabled Edge's built-in reading mode in a real-host check; direct `read:` launch follows Microsoft's documented Edge behavior and does not invoke the paid Azure service. Reports can contain sensitive chat content while their URL remains live.
...
- Read aloud automatically uses a matching voice advertised `localService=true`. Its context menu can select any voice exposed by WebView2 by exact `voiceURI`; Narrator-only and Edge-internal voices are not exposed. No autoplay or streaming speech.
